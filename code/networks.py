...
class Normalization(nn.Module):
    def __init__(self, device, dataset):
        ...
        if dataset == "mnist":
            ...
            self.mean = (
                torch.FloatTensor([0.4914, 0.4822, 0.4465])
                .view((1, 3, 1, 1))
                .to(device)
            )
            self.sigma = (
                torch.FloatTensor([0.2023, 0.1994, 0.201]).view((1, 3, 1, 1)).to(device)
            )
        else:
            assert False
...
class FullyConnected(nn.Module):
    def __init__(
        self, device, dataset, input_size, input_channels, fc_layers, act="relu"
    ):
...
def get_network(device, net):
    if net == "net1":
        ...
        return Conv(
            device, "mnist", 28, 1, [(16, 4, 2, 1), (32, 4, 2, 1)], [100, 10], 10
        )
    elif net == "net6":
        return Conv(
            device, "cifar10", 32, 3, [(16, 4, 2, 1), (32, 4, 2, 1)], [100, 10], 10
        )
    elif net == "net7":
        return Conv(
            device, "mnist", 28, 1, [(16, 4, 2, 1), (64, 4, 2, 1)], [100, 100, 10], 10
        )
    elif net == "net8":
        return ResNet(
            BasicBlock,
